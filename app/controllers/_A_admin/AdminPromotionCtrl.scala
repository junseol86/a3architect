...
//
//    if (user_data.length > 0 && user_data(0)("tbl_user.user_group") == "ADMIN")
//      Ok(views.html._A_admin_promotion_write(page_data, user_data, story))
//    else
//      Redirect("/")
//  }
//
...
//    story = promotionMdl.getAContractStory(idx)
//
//    if (user_data.length > 0 && user_data(0)("tbl_user.user_group") == "ADMIN")
//      Ok(views.html._A_admin_promotion_write(page_data, user_data, story))
//    else
...
//
//    if (insertOrModify == 0) {
//      val insert = promotionMdl.promotionWrite(fp.get("category"), fp.get("title"), fp.get("ir1"), fp.get("modified"))
//      if (user_data.length > 0 && user_data(0)("tbl_user.user_group") == "ADMIN")
//        Ok(insert match {
//          case Some(i: Long) => views.html.alert_and_move("게시물이 등록되었습니다.", "/admin/promotion")
//          case None => views.html.alert_and_move("에러가 발생하여 게시물이 등록되지 않앗습니다.", "/admin/promotion_write")
//        })
//      else
//        Redirect("/")
//    }
//    else {
//      val modify = promotionMdl.promotionModify(insertOrModify.toString, fp.get("category"), fp.get("title"), fp.get("ir1"), fp.get("modified"))
//      if (user_data.length > 0 && user_data(0)("tbl_user.user_group") == "ADMIN")
//        Ok(modify match {
//          case 1 => views.html.alert_and_move("게시물이 수정되었습니다", "/admin/promotion")
//          case default => views.html.alert_and_move("에러가 발생하여 게시물이 수정되지 않앗습니다.  다시 시도해 주십시오.", "/admin/promotion_write")
//        })
//      else
//        Redirect("/")
//    }
...
//        case default => views.html.alert_and_move("에러가 발생하여 게시물이 삭제되지 않앗습니다.  다시 시도해 주십시오.", "/admin/promotion_write")
//      })
//    else
//      Redirect("/")
//  }
